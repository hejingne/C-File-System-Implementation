...
int get_size_in_last_blk(a1fs_inode *ino) {
	...
	return (ino->size % A1FS_BLOCK_SIZE == 0)
			? A1FS_BLOCK_SIZE - (ino->size % A1FS_BLOCK_SIZE) // bytes_in_last_blk == 4096 when file or directory content
			: ino->size % A1FS_BLOCK_SIZE;				// has reached the end of its last data blk
}
...
void *get_ptr_to_byte_in_file(fs_ctx *fs, a1fs_inode *file_ino, int byte_index){
	int db_no = 0;
	int db_number_in_file = (byte_index % A1FS_BLOCK_SIZE == 0)		// represents the number of data blks
					? byte_index / A1FS_BLOCK_SIZE					// that we need to scan to find the byte at index byte_index
					: (byte_index / A1FS_BLOCK_SIZE) + 1;
	a1fs_extent *exts_blk = get_exts_blk(fs, file_ino);
	int counter = 0;
	for(int i = 0; i < (int)(file_ino->extents_count); i++){
		a1fs_extent extent = exts_blk[i];
		counter += extent.count;
		if (counter >= db_number_in_file) {
			db_no = extent.start + (counter - db_number_in_file);
			break;
		}
	}

	void *db = get_db(fs, db_no);
	void *ptr_to_byte_in_file = db + byte_index % A1FS_BLOCK_SIZE;
	return ptr_to_byte_in_file;
}
...
int extend_file(fs_ctx *fs, a1fs_inode *file_ino, int additional_bytes) {
	...
	while (additional_bytes != 0) {
			if (file_ino->size % A1FS_BLOCK_SIZE == 0) {	// Means new db is needed to store the bytes
				...
				add_bytes = A1FS_BLOCK_SIZE - (file_ino->size % A1FS_BLOCK_SIZE);
			}
                     ...
                     struct fuse_file_info *fi)
{
	...
	if (readable_size > (int)size) {
		memcpy(ptr_to_offset, buf, size);
		return size;
	} else {
		memcpy(ptr_to_offset, buf, readable_size);
